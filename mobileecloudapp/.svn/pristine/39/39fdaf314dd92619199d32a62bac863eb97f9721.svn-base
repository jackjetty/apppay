...
public class ICDealOrderParser extends BaseParser<ICDealOrderObject>  { 
		@Override
		public ICDealOrderObject parseJSON(String paramString)  { 
			checkResponse(paramString);  
			if (this.connectErrorObject.getErrCode()==0) {   
				try{
					
					return JSON.parseObject(resultData, ICDealOrderObject.class);
				}catch(Exception ex){ 
