...
import com.rising.appserver.service.UserLoginService;
...
public class UserLogin extends BaseAction {
	...
	public void userLogin(HttpServletRequest request,
		...
		try {
			...
			resultGson = userLoginService.userLogin(parameter,request);
			 
		} catch (DocumentException e) {
			result.put("respCode", -101);
			result.put("respInfo", "App发送xml报文格式出错!");
			resultGson = new Gson().toJson(result);
			log.error("userLogin()->App发送xml报文格式出错!" + e.toString());
		} catch (IOException e) {
			result.put("respCode", -200);
			result.put("respInfo", "读取请求xml报文出错！");
			resultGson = new Gson().toJson(result);
			log.error("userLogin()->读取请求xml报文出错！" + e.toString());
		}
