...
import com.rising.appserver.service.UserChangePasswordService;

@Controller
@RequestMapping("/user")
public class UserChangePassword extends BaseAction {

	Log log = LogFactory.getLog(UserChangePassword.class);
	@Autowired
	...
	public void userChangePassword(HttpServletRequest request,
		...
		try {
			...
			resultGson = userChangePasswordService
					.userChangePassword(parameter,request);
			 
		} catch (DocumentException e) {
			result.put("respCode",-101);
			result.put("respInfo", "App发送xml报文格式出错!");
			resultGson = new Gson().toJson(result);
			log.error("userChangePassword()->App发送xml报文格式出错!"+e.toString());
		} catch (IOException e) {
			result.put("respCode",-200);
			result.put("respInfo", "读取请求xml报文出错！");
			resultGson = new Gson().toJson(result);
			log.error("userChangePassword()->读取请求xml报文出错！"+e.toString());
		}
