...
public class BaseService {
	...
	@Value("#{propertiesReader[BaseAddress]}")
	...
	protected String redirectToMoblilePayMentService(String requestUrl,
		...
		URL url;
		try {
			url = new URL(BaseAddress + requestUrl);
			// 打开连接
			...
			if (conn.getResponseCode() == 200) {
				...
				result.put("respInfo", "网络繁忙，请稍等！");
				responseGson = new Gson().toJson(result);
				responseGson = URLEncoder.encode(responseGson, "UTF-8");
	...
	protected BufferedImage redirectToMoblilePayMentService2(String getImageURL2,
			String parameter,HashMap<String,Object> result) {
		URL url;
		try {
			url = new URL(BaseAddress + getImageURL2);
			// 打开连接
			HttpURLConnection conn = (HttpURLConnection) url.openConnection();
			// 设置提交方式
			conn.setDoOutput(true);
